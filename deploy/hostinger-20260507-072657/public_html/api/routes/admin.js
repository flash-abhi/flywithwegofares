...
let seoSettings = {
  pageTitle: 'Best Flight Deals, Hotels & Cruises | WeGo Fares',
  metaDescription: 'Find the best flight deals, hotels, cruises, and vacation packages at unbeatable prices. Book your next adventure with flywithwegofares.com - Your trusted travel partner.',
  keywords: 'flight deals, cheap flights, hotels, cruises, vacation packages, travel deals, airline tickets',
  ogImage: 'https://flywithwegofares.com/og-image.jpg',
  canonicalUrl: 'https://flywithwegofares.com'
};
...
let websiteContent = {
  ...
  heroSubtitle: '',
  aboutText: '',
  contactEmail: 'info@flywithwegofares.com',
...
let gscSettings = {
  ...
  siteUrl: process.env.GSC_SITE_URL || 'https://flywithwegofares.com',
  enabled: false,
  connected: false
};
...
router.get('/seo/audit', verifyAdminToken, async (req, res) => {
  try {
    const siteUrl = process.env.GSC_SITE_URL || 'https://flywithwegofares.com';
...
router.post('/blog/ai/generate', verifyAdminToken, async (req, res) => {
  try {
    ...
    const blogUrl = `https://flywithwegofares.com/blog/${aiPost.slug}`;
